...
<script>
    ...
    <script>
        ...
        // Tambahkan kode ini setelah prices definition
document.addEventListener("DOMContentLoaded", function () {
    // Event listener untuk service type dropdown
    document.getElementById("serviceType").addEventListener("change", function() {
        updatePriceDisplay();
        calculateSubtotal();
    });
    
    // Event listener untuk weight input
    document.getElementById("serviceWeight").addEventListener("input", function() {
        formatNumber(this);
        calculateSubtotal();
    });
...
// Fungsi untuk menampilkan harga per unit
function updatePriceDisplay() {
    ...
    if (serviceType && prices[serviceType]) {
        ...
        const unit = serviceType.includes("Sepatu") ? "pasang" : 
                    serviceType.includes("Karpet") ? "m²" : "kg";
        
        priceDisplayElement.innerHTML = `
            <strong>Harga: Rp ${price.toLocaleString()} per ${unit}</strong>
        `;
...
// Fungsi untuk menghitung subtotal secara real-time
function calculateSubtotal() {
    ...
    if (serviceType && weightValue && weight > 0 && prices[serviceType]) {
        const price = prices[serviceType];
        const subtotal = price * weight;
        
        subtotalElement.innerHTML = `
            <div style="background: #e6f3ff; padding: 10px; border-radius: 8px; margin-top: 10px;">
